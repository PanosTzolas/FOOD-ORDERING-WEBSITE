...
?>
    ...
    <script>
	 ...
	 var geocoder;
			var start_lat ;
			var start_lng;
				if( isNaN(parseFloat(getCookie("user_lat"))) || isNaN(parseFloat(getCookie("user_lng")))){
				...
				start_lng = 21.734573;
					...
					try{
					var start_address = getCookie("user_address");
			start_lat = parseFloat(getCookie("user_lat"));
			start_lng = parseFloat(getCookie("user_lng"));
  ...
  geocoder.geocode({
    ...
    infowindow.open(map, marker);
	createCookie("user_address", marker.formatted_address, "1");
	createCookie("user_lat", lat, "1");
	createCookie("user_lng", lng, "1");
  });
}
...
google.maps.event.addListener(marker, 'dragend', function() {
    ...
    geocodePosition(marker.getPosition());
        ...
        autocomplete.addListener('place_changed', function() {
		 ...
		 infowindowContent.children['place-lng'].textContent = lng;
          infowindow.open(map, marker);
		  	createCookie("user_address", address, "1");
			createCookie("user_lat", lat, "1");
			createCookie("user_lng", lng, "1");
        });
...
<?php
											...
											{
											?>
						<form class="form-wrap text-center" action="order.php" method="POST" >

							<input type="text" class="form-control" name="door_name" placeholder="Door Name" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Door Name'" >
							<input type="text" class="form-control" name="floor" placeholder="Floor" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Floor'" >
							<input type="hidden" name="summary" value="<?php echo $total; ?>" />
							<input type="submit" name="submit" value="Complete order" class="genric-btn primary">
						</form>
							<?php
											}
											?>
